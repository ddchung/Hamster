...
namespace Hamster
{
    ...
    public:
         ...
         */
        virtual ssize_t recvfrom(const IOVec *buf, size_t count, int flags, sys_sockaddr *addr, sys_socklen_t *addrlen) = 0;
         ...
         * @return A new BaseSocket for the connection, or nullptr on error and set `error`
         * @note If no incoming connection is present, error with `H_EAGAIN`
         */
        virtual BaseSocket *accept(sys_sockaddr *addr, sys_socklen_t *addrlen) = 0;
