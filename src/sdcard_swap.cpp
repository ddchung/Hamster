...
int Hamster::_swap_out(int index, const uint8_t *data)
{
    ...
    {
        Hamster::_trace("%s:%d: SD swapping failed to open file %s\n", __FILE__, __LINE__, name_buffer);
        return -1;
    }
    if (file.write(data, HAMSTER_PAGE_SIZE) != HAMSTER_PAGE_SIZE)
    {
        Hamster::_trace("%s:%d: SD swapping failed to write file %s\n", __FILE__, __LINE__, name_buffer);
        file.close();
        return -1;
    }
    file.close();
...
int Hamster::_swap_in(int index, uint8_t *data)
{
    ...
    {
        Hamster::_trace("%s:%d: SD swapping failed to read file %s\n", __FILE__, __LINE__, name_buffer);
        file.close();
        return -1;
    }
    file.close();
