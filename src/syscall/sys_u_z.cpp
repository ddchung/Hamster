...
namespace Hamster
{
    namespace
    {
        int do_write(Task &task, BaseTaskFD *fd, uint32_t buf_loc, uint32_t count)
        {
            ...
            while (count > 0)
            {
                ...
                ssize_t res = fd->write(it, to_write);
                if (res < 0)
                {
                    if (written > 0)
                        return written;
        ...
        int do_writev(Task &task, BaseTaskFD *fd, uint32_t vec_loc, uint32_t vlen)
        {
            uint32_t written = 0;
            while (vlen > 0)
            {
                ...
                if (vec.size > 0)
                {
                    int res = do_write(task, fd, vec.data, vec.size);
                    if (res < 0)
                    {
                        if (written > 0)
                            return written;
    ...
    int32_t sys_write(Task &task, int32_t task_fd, uint32_t buf_loc, uint32_t count)
    {
        ...
        return cvt_error(task.block([](Task &task, uint32_t task_fd, uint32_t buf_loc, uint32_t count, uint32_t, uint32_t, uint32_t){
    ...
    int32_t sys_writev(Task &task, int32_t task_fd, uint32_t vec_loc, uint32_t vlen)
    {
        ...
        return cvt_error(task.block([](Task &task, uint32_t task_fd, uint32_t vec_loc, uint32_t vlen, uint32_t, uint32_t, uint32_t){
            BaseTaskFD *fd = task.get_fd(task_fd);
            if (!fd)
                return -1;
